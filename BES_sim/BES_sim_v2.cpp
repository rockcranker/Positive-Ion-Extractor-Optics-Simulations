...
void simu( int argc, char **argv )
{
  ...
  double acceptance = 4e-3;
  double optic_rad = 4e-3;
  double beam_lower_bound = -0.02;
  double xT = 0.3;
  double xTl = xT - beam_lower_bound;
  double theta = 65 * M_PI/180;
  double del = 0.005;
  ...
  TCanvas *ch = new TCanvas("ch", "Canvas BES Hist", 2000, 1000);
  h1->Draw("hist");
  line_ha->Draw();
  ...
  for(size_t i = 1; i < nbins+1; i++){
    low_edge = h1->GetBinLowEdge(i);
    high_edge = h1->GetBinLowEdge(i+1);
    bin_counts = h1->GetBinContent(i);
