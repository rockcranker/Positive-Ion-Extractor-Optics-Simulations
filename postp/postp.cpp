...
void simu( int argc, char **argv )
{
  ...
  for( size_t k = 0; k < pmax; k++ ) {
    ...
    // count particles that are actually extractred
  ...
  auto x_div = root_hist_2xGauss( xAng, nbins, "hdx", "X Divergence All", "output.root", true, npart * 7/nbins, 0, 0.001, npart/nbins, 0.05);
  double wx = width(get<0>(x_div), get<1>(x_div), get<2>(x_div), get<3>(x_div), get<4>(x_div));

  auto y_div = root_hist_2xGauss( yAng, nbins, "hdy", "Y Divergence All", "output.root", true, npart*5/nbins, 0, 0.01, npart/nbins, 0.02);
  ...
  TCanvas *c2 = new TCanvas("c2", "Canvas 2", 2000, 1000);
  g->Draw("AP");
  ...
  for( int j = 1; j < 4; j++){
    ...
    for(int k = 0; k < npart; k++){
      if(pmn[k] == j){
	xcoord[partcount] = px[k];
	ycoord[partcount] = py[k];
	partcount++;
      }
    }
    delete g3;
    g3 = new TGraph(partcount, xcoord, ycoord);
    c2->cd();
    c2->Clear();
    g3->Draw("AP");
    g3->GetXaxis()->SetLimits(-0.5*size_x, 0.5*size_x);
    ...
    c2->Modified();
    c2->Update();
    c2->SaveAs(("beam_out_" + to_string(j) + ".png").c_str());
    species_out[j-1] = partcount;
  }

  ofstream file3( "results.txt" );
  file3 << "N beam: " << pdbrel.size() << "\n";
  file3 << "N transmitted: " << npart << "\n";
  file3 << "H1: " << species_out[0] << "\tH2: " << species_out[1] << "\tH3: " << species_out[2] << "\n";
  ...
  std::vector<std::string> filesToMove = {"geom.dat", "pdb.dat", "epot.dat", "angles.txt", "parameters.txt", "particles_out.txt",
					  ...
					  "beam_out_1.png", "beam_out_2.png", "beam_out_3.png" 
  };
