...
const double wBeam = 0.044;
const double hBeam = 0.008;
const double wSlit = 0.036;
const double hSlit = 0.004;
const double lMesh = 0.0001;
const int nCellW = (int)(wSim/lMesh);
const int nCellH = (int)(hSim/lMesh);
const int nCellL = (int)(lSim/lMesh);
const int ppc = 30;
const int N = nCellW * nCellH * ppc;
const int NSlit = (int)(wSlit/lMesh) * (int)(hSlit/lMesh) * ppc;
const int NBeam = (int)(wBeam/lMesh) * (int)(hBeam/lMesh) * ppc;
...
{
  Geometry geom( MODE_3D, Int3D(nCellW + 1, nCellH + 1, nCellL + 1), Vec3D(-wSim/2,-hSim/2,0), lMesh);
	...
	pdb.clear();
	pdb.add_rectangular_beam_with_energy( NBeam/3, J/3, 1.0, 1.0, 
					      5.0, Ti, Ti, 
					      Vec3D(0,0,0), Vec3D(1,0,0), Vec3D(0,1,0), 
					      wBeam/2, hBeam/2);
	pdb.add_rectangular_beam_with_energy( NBeam/3, J/3, 1.0, 2.0, 
					      5.0, Ti, Ti, 
					      Vec3D(0,0,0), Vec3D(1,0,0), Vec3D(0,1,0), 
					      wBeam/2, hBeam/2);
	pdb.add_rectangular_beam_with_energy( NBeam/3, J/3, 1.0, 3.0, 
					      5.0, Ti, Ti, 
					      Vec3D(0,0,0), Vec3D(1,0,0), Vec3D(0,1,0), 
					      wBeam/2, hBeam/2 );
